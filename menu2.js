...
const context = (jid, img, bot) => ({
    mentionedJid: [jid],
    isForwarded: true,
    forwardingScore: 1,
    forwardedNewsletterMessageInfo: {
        newsletterJid: bot.config.info.idChannel,
        newsletterName: bot.config.info.nameChannel,
        serverMessageId: 0
    },
    externalAdReply: {
        title: `${bot.config.info.nameBot} 🦅 | بوت مبني على إطار عمل MeowSab`,
        body: "بوت واتساب سهل التعديل وسريع جداً",
        thumbnailUrl: img,
...
> *_اختار قسم من القائمة عشان يبعتلك اوامر القسم_*`;
        ...
        await conn.sendButtonNormal(m.chat, {
            ...
            mentions: [m.sender],
            newsletter: {
                name: bot.config.info.nameChannel,
                jid: bot.config.info.idChannel
            }
        }, global.reply_status);
        return;
    }
    ...
    if (!cat) {
        await conn.sendMessage(m.chat, { text: '*❌ اختار رقم صحيح من 1 لـ 18*', contextInfo: context(m.sender, getImg(bot), bot) }, { quoted: m });
        return;
    }
    ...
    if (!categoryCmds.length) {
        await conn.sendMessage(m.chat, { text: '*❌ القسم فاضي*', contextInfo: context(m.sender, getImg(bot), bot) }, { quoted: m });
        return;
    }
...
╭─┈─┈─┈─⟞${cat[3]}⟝─┈─┈─┈─╮
┃ *⌯︙Aisa v1 ~ ${bot?.config?.info?.nameBot || 'اسيا'}*
╰─┈─┈─┈─⟞${cat[3]}⟝─┈─┈─┈─╯
> *رَبَّنَا اغْفِرْ لَنَا وَلِإِخْوَانِنَا*`.trim(), contextInfo: context(m.sender, getImg(bot), bot) }, { quoted: m });
}

handler.command = ['المهام', 'الاوامر', 'القائمة'];
